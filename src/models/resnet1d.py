...
from src.models.basic_conv1d import create_head1d
...
class Bottleneck1d(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, kernel_size=3, downsample=None):
        ...
        self.conv2 = nn.Conv1d(
            planes, planes, kernel_size=kernel_size, stride=stride, padding=(kernel_size - 1) // 2, bias=False
        )
        self.bn2 = nn.BatchNorm1d(planes)
...
class BasicBlock1dwrn(nn.Module):
    def __init__(self, ni, nf, stride, drop_p=0.0, ks=3):
        ...
        self.shortcut = (
            conv1d(ni, nf, ks[1], stride) if (ni != nf or stride > 1) else noop
        )  # adapted to make it work for fix_feature_dim=True
...
class WideResNet1d(nn.Sequential):
    def __init__(
        ...
        for i in range(num_groups):
            n_channels.append(start_nf if fix_feature_dim else start_nf * (2**i) * k)

        layers = [conv1d(input_channels, n_channels[0], 3, 1)]  # conv1 stem
        for i in range(num_groups):
            layers += _make_group(
                N, n_channels[i], n_channels[i + 1], BasicBlock1dwrn, (1 if i == 0 else 2), drop_p, ks=kernel_size
            )
...
def wrn1d_22(**kwargs):
    return WideResNet1d(num_groups=3, N=3, k=6, drop_p=0.0, **kwargs)
