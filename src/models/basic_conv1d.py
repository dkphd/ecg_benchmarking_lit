...
def listify(p=None, q=None):
    "Make `p` a list and the same length as `q`."
    if p is None:
        p = []
        ...
        for i in range(len(filters)):
            ...
            layers_tmp.append(
                _conv1d(
                    ...
                    dilation=dilation,
                    act="none"
                    if ((headless is True and i == len(filters) - 1) or (split_first_layer is True and i == 0))
                    else act,
            ...
            if split_first_layer is True and i == 0:
                layers_tmp.append(_conv1d(filters[0], filters[0], kernel_size=1, stride=1, act=act, bn=bn, drop_p=0.0))
